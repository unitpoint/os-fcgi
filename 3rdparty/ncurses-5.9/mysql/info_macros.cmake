...
SET(VERSION "5.5.32")
SET(CMAKE_CURRENT_SOURCE_DIR "/tmp/os-igor/mysql")
SET(CMAKE_CURRENT_BINARY_DIR "/tmp/os-igor/ncurses-5.9/mysql")
...
MACRO(CREATE_INFO_SRC target_dir)
  ...
  IF(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/.bzr)
    ...
    EXECUTE_PROCESS(
      COMMAND ${BZR_EXECUTABLE} version-info ${CMAKE_CURRENT_SOURCE_DIR}
  ...
  ELSEIF(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/Docs/INFO_SRC)
    ...
    FILE(READ ${CMAKE_CURRENT_SOURCE_DIR}/Docs/INFO_SRC SOURCE_INFO)
    FILE(WRITE ${INFO_SRC} "${SOURCE_INFO}\n")
  ELSEIF(EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/INFO_SRC)
    # This is not the proper location, but who knows ...
    FILE(READ ${CMAKE_CURRENT_SOURCE_DIR}/INFO_SRC SOURCE_INFO)
    FILE(WRITE ${INFO_SRC} "${SOURCE_INFO}\n")
  ELSE()
    # This is a fall-back.
    FILE(WRITE ${INFO_SRC} "\nMySQL source ${VERSION}\n")
...
MACRO(CREATE_INFO_BIN)
  ...
  IF(EXISTS ${CMAKE_CURRENT_BINARY_DIR}/CMakeCache.txt)
    ...
    EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} -N -L ${CMAKE_CURRENT_BINARY_DIR} OUTPUT_VARIABLE FEATURE_FLAGS)
